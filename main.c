...
#define ADVERTISING_LED_PIN_NO               LED_2                                      /**< Is on when device is advertising. */
...
static uint8_t                     adc = 0xAA; // Adc initail value					
uint8_t abc[1];																	 
...
static void leds_init(void)
{
    ...
    nrf_gpio_cfg_output(CONNECTED_LED_PIN_NO);
		nrf_gpio_cfg_output(LED_4);                   //Added for ADC operation
...
static void adc_sampling_timeout_handler(void * p_context)
{
	...
	//nrf_gpio_pin_toggle(LED_4);		//Toggle LED2 to indicate start of sampling
...
static void battery_level_meas_timeout_handler(void * p_context)
{
    ...
    NRF_ADC->EVENTS_END = 0;	
    // NORDIC adc value is updated here
    abc[0] = NRF_ADC->RESULT; 
    NRF_ADC->TASKS_STOP = 1;//Use the STOP task to save current. Workaround for PAN_028 rev1.5 anomaly 1.
		
    adc_data.company_identifier           = 0xABCD; //0x0059; // Nordics company ID
    adc_data.data.p_data                  = abc;
